...
final class Graph2Data {
    ...
    private let userNode = { (user: User) -> SKShapeNode in
        ...
        // Image
        if let image = UIImage(named: user.imageName) {
    ...
    private let vertexNode = { (vertex: Vertex) -> SKShapeNode in
        ...
        // Image
        if let image = UIImage(named: vertex.imageName) {
            let imageNode = SKSpriteNode(texture: SKTexture(image: image))
            imageNode.size     = CGSize(width: 98, height: 98)
            imageNode.position = CGPoint(x: 0, y: 10)
            shapeNode.addChild(imageNode)
    ...
    func request() {
        ...
        do {
            ...
            // User
            let userNode = userNode(data.user)
            nodes.append(userNode)
            
            guard let userNodeBody = userNode.physicsBody else { return }
            ...
            for (i, graph) in data.user.graphs.enumerated() {
                ...
                let joint = SKPhysicsJointSpring.joint(withBodyA: userNodeBody, bodyB: nodeBody, anchorA: userNode.position, anchorB: node.position)
                joint.damping   = 0.3
                joint.frequency = 0.8
